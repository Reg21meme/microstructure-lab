...
Usage:
    python3 -m mslab.backtest.run_cpp_sim [SYMBOL]
"""
...
def make_sim(latency_ms: float = LATENCY_MS,
             ...
             symbol: str = SYMBOL) -> mb.ExecutionSim:
    if fee_cfg is None:
...
def run_simulation(latency_ms: float = LATENCY_MS,
    ...
    print(f"  Symbol={symbol}  Latency={latency_ms}ms")
    ...
    upd_rows      = load_updates(symbol)
    snap_rows     = load_snapshot_rows(symbol)
    test_start_ts = test["ts_local"].iloc[0]
    print(f"  Test period starts at ts={test_start_ts}")

    if fee_cfg is None:
        fee_cfg = load_fees(FEE_SCENARIO)
    sim = make_sim(latency_ms, fee_cfg, symbol)
...
def run(symbol: str = SYMBOL) -> None:
    print("=" * 60)
    print(f"EXECUTION SIMULATION — {symbol}")
    print("=" * 60)
...
if __name__ == "__main__":
    sym = sys.argv[1] if len(sys.argv) > 1 else SYMBOL
    run(sym)
